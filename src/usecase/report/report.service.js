...
export const reportService = {
    async getSalesReport(period, startDate, endDate) {
        ...
        const grossSales = orders.reduce((sum, order) => sum + parseFloat(order.total), 0);
        ...
        const discount = orders.reduce((sum, order) => {
            if (order.discountId && order.discount) {
                ...
                if (order.discount.type === 'PERCENT') {
                    discountAmount = (orderSubTotal * parseFloat(order.discount.value)) / 100;
        ...
        const refunds = await reportRepository.getRefundedOrdersCount(start, end);

        // Calculate net sales (gross - discount)
        const netSales = grossSales - discount;

        // Calculate expenses (sesuaikan persentase dengan kebutuhan bisnis)
        const expense = netSales * 0.03; // 3% dari net sales
        const operational = netSales * 0.18; // 18% dari net sales
        const tax = netSales * 0.02; // 2% pajak

        const totalCollected = netSales - expense - operational - tax;
